...
public class BalanceInquiryFrame extends JFrame
{
	...
	public BalanceInquiryFrame( int userAccountNumber, BankDatabase atmBankDatabase ) {
		super( "ATM - Balance Information" ); //title for the frame
	...
	private void centerCreater() {
		...
		for(int i = 0; i < message.length; i++) {
			switch(i) {
				case 0: 
					message[i] =  new JLabel(" - Available balance: HKD" + String.format("%,.2f",availableBalance));
					message[i].setFont(new Font("Tahoma", Font.BOLD, 15));
					break;
				case 1:
					message[i] =  new JLabel(" - Total balance:  HKD" + String.format("%,.2f",totalBalance));
					message[i].setFont(new Font("Tahoma", Font.BOLD, 15));
					break;
				case 2:
					message[i] =  new JLabel(" | Enter to Take your card |  Cancel to Return menu |");
